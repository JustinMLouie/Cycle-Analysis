...
for n = 5:5
    ...
    diff80Cutoff = zeros(numCycles - 2, 1);
    ...
    for i = 2:numCycles - 1 % iterate through cycles
        ...
        chargeRegionData = cycData(cycData(:, 7) > 0.00002, :); % Data in charging region, theoretical > 0, realistic > 0.0002
        % chargeRegionData = chargeRegionData(chargeRegionData(:, 6) == stepIndexVals, :);

        % if i == 2
        %     figure();
        %     plot(cycData(:, 3), cycData(:, 19), 'b'); hold on;
        %     plot(chargeRegionData(:, 3), chargeRegionData(:, 19), 'r');
        %     xlabel("Time");
        %     ylabel("Pressure");
        % end

        
        timeVals = chargeRegionData(:, 3); % Step time values, s
        % 
        % if i == 18
        %     fprintf("stop here")
        % end

        timeVals = timeVals - min(timeVals);
        ...
        chargeDataDetrend = cycDataDetrend(cycDataDetrend(:, 7) > 0.00002, :); % Data in charging region, theoretical > 0, realistic > 0.0002
        ...
        timeValsDetrend = chargeDataDetrend(:, 3);
        timeValsDetrend = timeValsDetrend - min(timeValsDetrend);
        ...
        diff80Cutoff(i - 1) = minPNoFit(7) - minP(7);

        if i == 18
            figure();
            plot(pressureTime(:, 1), pressureTime(:, 2), 'b', 'LineWidth', 2); hold on;
            yline(minPNoFit(4), '--b', 'LineWidth', 2.0);
            yline(minPNoFit(5), '--b', 'LineWidth', 2.0);
            plot(pFitTimePressure(:, 1), pFitTimePressure(:, 2), 'r', 'LineWidth', 2.0); hold on;
            yline(minP(4), '--r', 'LineWidth', 2.0);
            yline(minP(5), '--r', 'LineWidth', 2.0);
            xlabel("Time");
            ylabel("Pressure");
            legend("Unfitted", "50% Unfitted Cutoff", "60% Unfitted Cutoff", "Fitted, ", "50% fitted Cutoff", "60% Fitted Cutoff");
        ...
        % if i == 18 
        %     figure();
        %     scatter(pressureTime(:, 1), pressureTime(:, 2), 'LineWidth', 2); hold on;
        %     plot(pressureTime(:, 1), pressureTime(:, 2), 'LineWidth', 2); 
        %     % xlabel("Time");
        %     % ylabel("Pressure")
        %     % title("Rawdata " + n + " Cycle " + i)
        % end
        ...
        % if i == 17
        %     figure()
        %     scatter(timeVals, pressureVals, 'LineWidth', 2); hold on;
        %     plot(pFitTimePressure(:, 1), pFitTimePressure(:, 2), 'LineWidth', 2);
    ...
    % figure();
    % plot(2:numCycles - 1, diff80Cutoff);
    % xlabel("Cycle");
    % ylabel("diff 80% Cutoff") 
